...
PARENT_MATCHER = re.compile(r"(?P<parent>.*?) bags contain")
CHILD_MATCHER = re.compile(r"(?P<cost>\d) (?P<child>.*?) bags?")


class Bag:
    name: str
    children: Dict["Bag", int]
    parents: Set["Bag"]

    def __init__(self, name: str, _permission=False):
        if not _permission:
            raise TypeError("Don't create these yourself, use the BagDict.")
        self.name = name
        self.children = {}
        self.parents = set()
...
class BagDict(dict):
    def __missing__(self, key):
        self[key] = Bag(key, _permission=True)
        return self[key]


def parse(inp) -> Dict[str, Bag]:

    bags: Dict[str, Bag] = BagDict()
    for line in inp:
        parent = bags[PARENT_MATCHER.match(line).group("parent")]

        for match in CHILD_MATCHER.finditer(line):
            child, cost = bags[match.group("child")], int(match.group("cost"))
            parent.children[child] = cost
