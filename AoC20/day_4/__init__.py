...
HCL_MATCHER = re.compile(r"#[a-f0-9]{6}")
PID_MATCHER = re.compile(r"\d{9}")
HGT_CM_MATCHER = re.compile(r"(\d{3})cm")
HGT_IN_MATCHER = re.compile(r"(\d{2})in")


@dataclass
class Passport:
    validate: InitVar[bool]
    ...
    def __post_init__(self, validate):
        ...
        elif match := HGT_IN_MATCHER.fullmatch(self.hgt):
            hgt = 59 <= int(match.group(1)) <= 76
        else:
            hgt = False

        hcl = HCL_MATCHER.fullmatch(self.hcl) is not None
        ecl = self.ecl in {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}
        pid = PID_MATCHER.fullmatch(self.pid) is not None

        if not all([byr, iyr, eyr, hgt, hcl, ecl, pid]):
            raise ValueError(f"{[*zip(vars(self).items(), [byr, iyr, eyr, hgt, hcl, ecl, pid])]}")


def parse(inp, validate):
    acc = [""]
    for line in inp.splitlines():
        if not line:
            ...
            acc[-1] += " " + line
    for val in acc:
        attrs = {k: v for k, v in [elem.split(":") for elem in val.split(" ") if elem]}
        try:
            yield Passport(**attrs, validate=validate)
