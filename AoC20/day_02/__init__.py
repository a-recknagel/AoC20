...
class Rule:
    PARSE = re.compile(r"(?P<first>\d+)-(?P<second>\d+) (?P<char>\w): (?P<word>\w+)")

    def __init__(self, inp):
        parsed = Rule.PARSE.match(inp).groupdict()
        self.first = int(parsed["first"])
        self.second = int(parsed["second"])
        self.char = parsed["char"]
        self.word = parsed["word"]

    def check_1(self):
        return self.first <= self.word.count(self.char) <= self.second

    def check_2(self):
        return (self.word[self.first-1] == self.char) ^ (self.word[self.second-1] == self.char)
